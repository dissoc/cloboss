...
(ns cloboss.web.async-test
  (:require [clojure.test :refer :all]
            [cloboss.web :refer :all]
            [cloboss.web.async :refer :all]
        ...
        handler (fn [req]
                  (let [result (if-not (-> req :session :foo)
                                 ...
                                 (-> (response "boo")
                                   (assoc :session nil)))]
                    result))
        orig-get-or-create-session (deref #'cloboss.web.internal.undertow/get-or-create-session)]
    (with-redefs [cloboss.web.internal.undertow/get-or-create-session
                  (fn g-o-c-s

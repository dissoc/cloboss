...
(ns cloboss.web
  ...
  (:require [cloboss.internal.options :refer [boolify opts->set set-valid-options!
...
(defn run
  ...
  When `handler` is a Servlet, the following options are also supported:
   ...
   a global handler, see [[cloboss.web.middleware/wrap-write-error-handling]].
   ...
   keywords using the [[cloboss.web.undertow/options]] function. For
  ...
  [handler & options]
  (let [undertow-options-maybe (try-resolve 'cloboss.web.undertow/options-maybe)
        options (-> options
                  kwargs-or-map->map
...
(set-valid-options! run (-> (opts->set Web$CreateOption Web$RegisterOption)
                          (conj :contexts)
                          (boolify :dispatch)
                          (clojure.set/union (-> (try-resolve 'cloboss.web.undertow/options)
                                               meta
                                               :valid-options))))

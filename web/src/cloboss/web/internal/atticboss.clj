...
(ns ^:no-doc ^:internal cloboss.web.internal.atticboss
    (:require [cloboss.web.internal.servlet  :refer [create-servlet websocket-servlet-filter-map]]
              [cloboss.internal.options  :refer [boolify extract-options opts->set
...
(defn ^:internal server [opts]
  (AtticBoss/findOrCreateComponent Web
...
(defn ^:internal mount [^Web server handler opts]
  (let [hdlr (if (or (fn? handler)
               ...
               (if (in-container?)
                 ...
                 ((u/try-resolve 'cloboss.web.internal.undertow/create-http-handler)
                   handler))
               handler)
        ...
        (catch IllegalStateException e
          (if (re-find #"after servlet init" (.getMessage e))
            (throw (IllegalStateException.
                     "You can't call cloboss.web/run outside of -main inside the container."
                     e))
            (throw e))))

...
(ns ^{:no-doc true} cloboss.web.internal.servlet
  (:require [clojure.string :as str]
            [cloboss.web.internal.ring :as ring]
            [cloboss.web.internal.headers :as hdr]
            [cloboss.internal.util :refer [try-resolve warn]]
  ...
  (:import [org.projectodd.atticboss.web.async Channel
           ...
           [jakarta.servlet.http HttpServlet HttpServletRequest HttpServletResponse HttpSession]
           [jakarta.servlet Servlet ServletConfig ServletContext]
           [jakarta.websocket HandshakeResponse]
           [jakarta.websocket.server ServerContainer ServerEndpointConfig$Builder ServerEndpointConfig$Configurator]))
...
;; See also path-info' in cloboss.web.internal.undertow. We un-DRYed
...
(defn add-endpoint
  ...
  the same as the callbacks provided to [[cloboss.web.async/as-channel]]."
...
(defn async-streaming-supported? []
  ...
  (if-let [f (try-resolve 'cloboss.wildfly/async-streaming-supported?)]
    (f)
    true))

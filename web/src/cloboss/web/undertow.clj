...
(ns cloboss.web.undertow
  "Advanced options specific to the Undertow web server used by Immutant"
  (:require [cloboss.util :refer (at-exit)]
            [cloboss.internal.util :refer (kwargs-or-map->map)]
            [cloboss.internal.options :refer (validate-options set-valid-options! opts->set coerce)]
            [cloboss.web.internal.undertow :refer (create-http-handler)]
            [cloboss.web.ssl :refer (keystore->ssl-context)])
...
(defn ^HttpHandler graceful-shutdown
  ...
  `cloboss.util/at-exit` fn that prohibits new requests while waiting
...
(def ^:private non-atticboss-options
...
(defn ^:no-doc ^:internal options-maybe
  [opts]
  (if (some non-atticboss-options (keys opts))
    (options opts)
    opts))

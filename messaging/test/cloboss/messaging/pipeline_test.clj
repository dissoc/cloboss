...
(ns cloboss.messaging.pipeline-test
  (:require [cloboss.messaging.pipeline :refer :all]
...
(use-fixtures :once
  (fn [f]
    (u/reset-fixture f)
    (reset! @#'cloboss.messaging.pipeline/pipelines {})))
...
(deftest should-call-messaging-stop-with-the-pipeline-from-the-metadata
  (with-redefs [cloboss.messaging/stop (fn [q] (is (= :a-queue q)) :called)]
    (is (= :called (stop (with-meta #() {:pipeline :a-queue, :name "pipe"}))))))

(testing "pipeline"
  (deftest should-pass-a-queue-name-based-on-the-given-name
    (let [q-name (atom nil)]
      (with-redefs [cloboss.messaging/queue (fn [& args] (reset! q-name (first args)))
                    cloboss.messaging.pipeline/pipeline-listen (constantly nil)]
        (pipeline "my-pl")
  ...
  (deftest should-pass-durable-through-to-queue
    (let [q-args (atom nil)]
      (with-redefs [cloboss.messaging/queue (fn [& args] (reset! q-args args))
                    cloboss.messaging.pipeline/pipeline-listen (constantly nil)]
...
(testing "the returned value"
  ...
  (deftest should-have-the-listeners-as-metadata
    (with-redefs [cloboss.messaging.pipeline/pipeline-listen (constantly :l)]
...
(deftest reloading-pipeline-ns-should-not-reset-internal-state
  (let [name :reloading-ns-pl]
    (pipeline name)
    (require '[cloboss.messaging.pipeline :as pl] :reload-all)
    (is (thrown? IllegalArgumentException
          (pipeline name)))))

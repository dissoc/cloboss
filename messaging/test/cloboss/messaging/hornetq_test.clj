...
(ns cloboss.messaging.hornetq-test
  (:require [cloboss.messaging :as msg]
            [cloboss.messaging.hornetq :refer :all]
            [clojure.test :refer :all]))

(deftest normalize-destination-match-works
  (are [exp given] (= exp (#'cloboss.messaging.hornetq/normalize-destination-match given))
       "#"                   "#"
       "jms.queue.#"         "jms.queue.#"
  ...
  (doseq [match ["jms.#" "*" "foo"]]
    (is (thrown? IllegalArgumentException
          (#'cloboss.messaging.hornetq/normalize-destination-match match)))))

(deftest set-companion-options-works
  (are [exp given] (= exp (#'cloboss.messaging.hornetq/set-companion-options given))

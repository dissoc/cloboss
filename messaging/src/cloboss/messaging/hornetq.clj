...
(defn ^JMSServerManager server-manager
  ...
  []
  (if (pu/in-container?)
    (throw (IllegalStateException. "cloboss.messaging.artemis functions can't be used in-container"))
    (when-let [^ArtemisMessaging broker (broker nil)]
...
(defn destination-controller
  "Returns the destination controller for `destination`.
   ...
   [[cloboss.messaging/queue]] or [[cloboss.messaging/topic]].
  ...
  ([destination control-type]
     (when-let [hq-server (server-manager)]
       (.getResource (-> hq-server .getActiveMQServer .getManagementService)
...
(def ^:private address-settings-coercions
  {:address-full-message-policy (fn [addr]
                                  (u/when-import 'org.apache.activemq.artemis.core.settings.impl.AddressFullMessagePolicy
                                    (eval `(case (name ~addr)
...
(defn ^{:valid-options
  ...
  "Sets the HornetQ-specific address options for the given match.
   ...
   returned from [[cloboss.messaging/queue]] or
   [[cloboss.messaging/topic]], or a fully qualified jms destination
  ...
  (u/when-import 'org.apache.activemq.artemis.core.settings.impl.AddressSettings
    (when (seq settings)
      (.addAddressSettings
        (server-manager)
